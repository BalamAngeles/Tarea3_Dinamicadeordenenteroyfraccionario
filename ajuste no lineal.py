...
fig1 = plt.figure("v(t) teorica", figsize=(8, 5))
...
# El modelo se fuerza a pasar por el origen (v(0)=0), sin termino de
# intercepto, por lo que Mathematica calcula el R^2 respecto a CERO
# (SS_tot = sum(v^2)) y no respecto a la media.
ss_res = np.sum((v_data - v_pred) ** 2)
ss_tot = np.sum(v_data ** 2)
r2 = 1 - ss_res / ss_tot
...
# ----------------------------------------------------------------------
# 5. Grafica superpuesta (Show[p1, p2]): curva ajustada + puntos
# ----------------------------------------------------------------------
t_fit = np.linspace(0, 1.2, 200)
v_fit = modelo(t_fit, k_ajustado)

fig2 = plt.figure("Ajuste no lineal", figsize=(8, 5))
...
# ----------------------------------------------------------------------
# 6. Grafica de los puntos experimentales solos (ListPlot de d1)
# ----------------------------------------------------------------------
fig3 = plt.figure("Datos experimentales d1", figsize=(8, 5))
plt.scatter(t_data, v_data, color="C0", label="Datos experimentales d1")
plt.xlabel("t")
plt.ylabel("v[t]")
plt.title("ListPlot de los datos experimentales d1")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
print("\nPrograma terminado. Se mostraron 3 ventanas con las graficas.")
